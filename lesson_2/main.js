function init() {
	...
	// - trent: added spheres to light sources
	let lightSphereMat = getMaterial('basic', 'rgb(255,255,255)');
	let lightLSphere = getSphere(lightSphereMat, .1, 50);
	let lightRSphere = getSphere(lightSphereMat, .1, 50);
	lightLeft.add(lightLSphere)
	lightRight.add(lightRSphere)
	...
	// cube map
	const path = '/assets/cubemap/museum/';
	const format = '.jpg';
	const urls = [
	...
	// set cube to background of environment
